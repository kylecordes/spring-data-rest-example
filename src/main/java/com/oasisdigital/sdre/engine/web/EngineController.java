 ...
 */
package com.oasisdigital.sdre.engine.web;
...
import com.oasisdigital.sdre.RestbucksWebApplicationInitializer.WebConfiguration;
import com.oasisdigital.sdre.engine.Engine;
...
class EngineController implements ResourceProcessor<RepositoryLinksResource> {
	...
	private final @NonNull Engine processor;
	private final @NonNull PartRepository repository;

	/**
	 * Exposes all {@link Part}s currently in preparation.
	 * 
	 * @return
	 */
	@RequestMapping("/engine")
	HttpEntity<Resources<Resource<Part>>> showOrdersInProgress() {

		Resources<Resource<Part>> orderResources = Resources.wrap(processor.getOrders());
	...
	/**
	 * An additional resource exposing paged {@link Part}s to show the usage of both {@link Pageable} and
	...
	HttpEntity<PagedResources<Resource<Part>>> foo(Pageable pageable, PagedResourcesAssembler<Part> assembler) {

		Page<Part> orders = repository.findAll(pageable);
